...
[![Deploy to GitHub Pages](https://github.com/xuanyiying/apex-portfolio/actions/workflows/deploy.yml/badge.svg)](https://github.com/xuanyiying/apex-portfolio/actions/workflows/deploy.yml)
...
本项目已配置 GitHub Actions 自动部署工作流。只需简单的设置，即可实现 push 代码后自动更新线上站点。

1. **Fork 本仓库** 到你的 GitHub 账户。
2. 进入仓库的 **Settings** -> **Pages**。
3. 在 **Build and deployment** 下的 **Source** 选项中，选择 **GitHub Actions**。
4. 当你推送代码到 `main` 分支时，Action 会自动运行并将网站部署到 GitHub Pages。
5. 部署完成后，你的网站将可以通过 `https://<你的用户名>.github.io/apex-portfolio/` 访问。
...
2. 启动生产服务器 (仅用于本地预览构建结果)：

   ```bash
   npx serve out
   ```

   *注意：由于配置了静态导出 (`output: 'export'`)，`npm run start` 命令可能不适用，建议使用 `serve` 或部署 `out` 目录到任何静态文件服务器。*
