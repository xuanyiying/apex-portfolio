...
async function fetchGithubProjects() {
    ...
    try {
        ...
        for (const repo of repos) {
            ...
            // Fallback to topics/language if no dependencies found
            if (techStack.main.length === 0) {
                const fallbackTags = repo.topics && repo.topics.length > 0 ? repo.topics : (repo.language ? [repo.language] : []);
                techStack.main = fallbackTags;
            }
            ...
            projects.push({
                ...
                tags: [...techStack.main, ...techStack.tools],
                techStack, // Adding the structured tech stack
                github: repo.html_url,
                demo: repo.homepage || '',
                featured: repo.stargazers_count > 10 || repo.name === 'ezento' || repo.name === 'ai-ace-job',
